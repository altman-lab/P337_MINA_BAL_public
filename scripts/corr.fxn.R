...
corr.fxn <- function(counts, metadata, match.by="libID", corr="pearson", 
                     ...
                     heatmap=FALSE, dotplot=FALSE, color.visit=FALSE){
...
##### Combine count and meta data#####
  dat.all <- count.dat %>% 
    full_join(meta.dat) %>% 
    column_to_rownames(match.by) %>% 
    as.matrix()
  
print(paste("Correlating", nrow(dat.all), "samples", sep=" "))

##### Correlation #####

corr.result <- rcorr(dat.all, type=corr)

corr.R <- as.data.frame(corr.result$r) %>% 
  rownames_to_column() %>% 
  filter(rowname %in% colnames(count.dat)) %>% 
  dplyr::select(rowname, all_of(colnames(meta.dat))[-1]) %>% 
...
##### Plots #####
  if(heatmap){
    ...
    dir.create("figs/correlation/", showWarnings = FALSE)
  ...
  if(dotplot){
    ...
    #### dot plots ####
    print("Dot plots")
    plot_dat <- as.data.frame(count.dat) %>% 
      pivot_longer(-match.by, names_to = "name",values_to = "count.value")
    
    plot_dat <- as.data.frame(meta.dat) %>% 
      pivot_longer(-match.by, names_to = "meta", values_to = "meta.value") %>% 
      inner_join(plot_dat, by=match.by)
      ...
      for (count.var in unique(rownames(corr.R))){
        ...
        if(color.visit){
          ...
          plot1 <- ggplot(plot_dat.sub, 
            ...
            #Indiv patient points
            geom_point(aes(color=get(match.by))) +
            #Overall black trend line
            geom_smooth(formula=y~x,
                        color="black", method="lm", se=FALSE) +
            #Beautify
