...
plot0 <- dat.delta %>% 
  drop_na(`L vA insula\nBOLD score`, `Eosinophil\n cell %`) %>% 
  ggplot(aes(y=`L vA insula\nBOLD score`, x=`Eosinophil\n cell %`)) +
  geom_point() +
  geom_smooth(method="lm",se=FALSE, color="#FF8679") +
...
ggsave(plot_grid(plot00, NULL, row2, nrow=3, labels = c("(A)","","(B)"),
       ...
       filename = "publication/Fig4.correlation.png",
       height=5, width=7)
ggsave(plot_grid(plot00, NULL, row2, nrow=3, labels = c("(A)","","(B)"),
                 rel_heights = c(1, 0.05, 1)), 
       filename = "publication/Fig4.correlation.pdf",
       height=5, width=7)
