...
ggsave("publication/Fig3.STRING700_lrg.clust.pdf", plot.large.clust, 
       height=10, width=10)

#### Calculate centrality main cluster ####
btw <- as.data.frame(betweenness(subgraph.large.clust)) %>% 
  rename(btw=1) %>% 
  rownames_to_column("STRING_id")
degree <- as.data.frame(degree(subgraph.large.clust)) %>% 
  rename(degree=1) %>% 
  rownames_to_column("STRING_id")

graph.metric <- full_join(btw, degree) %>% 
  left_join(map) %>% 
  arrange(-btw)
