...
Compute mean absolute values (MAE) for variables on each component. Other metrics (MSE, R2, bias) were also explored and can be found in `results/PLS/`

```{r eval=FALSE}
#X variables
spls.MAE.X <- tune.spls(X, Y, 
                        ncomp = ncompX, #Determined above
                        test.keepX = c(1:30, 40),
                        validation = "Mfold", 
...
There is little reduction in MAE with the last several variables (features) on X component 1 and Y component 2 (as seen by differences near 0 below). Thus, variables on these components could be reduced without much error increase. However, further reduction in the X space results in EOS module 08 inclusion (not shown). As this module contains >100 genes, it actually complicates the space more than the ~10 cytokines included at MAE minimum. Therefore, MAE minimum will still be used.

```{r echo=FALSE, warning=FALSE, fig.height=3}
MAE.X <- as.data.frame(spls.MAE.X[["error.rate"]]) %>% 
  rownames_to_column("feature") %>% 
  #Difference
  mutate(across(comp1:comp3, ~.-lag(.))) %>% 
  pivot_longer(-feature)

MAE.X %>% 
  filter(name == "comp1") %>% 
  ggplot(aes(x=as.numeric(feature), y=value)) +
  geom_point() + 
  geom_path(aes(group=name)) +
  theme_bw() +
  facet_wrap(~name) +
  labs(x="Number of selected features",
       y="Change in MAE from 1 fewer features",title="X") +
  geom_hline(aes(yintercept=0), lty="dashed") +
  geom_vline(aes(xintercept=13), lty="dashed")

MAE.Y <- as.data.frame(spls.MAE.Y[["error.rate"]]) %>% 
  rownames_to_column("feature") %>% 
  #Difference
  mutate(across(comp1:comp3, ~.-lag(.))) %>% 
  pivot_longer(-feature)
...
MAE.Y %>% 
  ...
  facet_wrap(~name) +
  labs(x="Number of selected features",
       y="Change in MAE from 1 fewer features",title="Y") +
  geom_hline(aes(yintercept=0), lty="dashed") +
  geom_vline(aes(xintercept=6), lty="dashed")
```

Thus, these components will be reduced to features before this flattening, resulting in the following variables per component. Additionally, we will restrict analyses to components 1-2 as they explain >50% of the total variation.
...
#Save
save(result.spls, file="results/PLS/SPLS.RData")
```
                ...
                ifelse(space=="X" & comp=="comp2", 
                      paste("X\nComponent 2 = ",
                            round(result.spls$explained_variance$X[2]*100,
                                  digits=1),
                            "%", sep=""),
                # ifelse(space=="X" & comp=="comp3", 
                #       paste("X\nComponent 3 = ",
                #             round(result.spls$explained_variance$X[3]*100,
                #                   digits=1),
                #             "%", sep=""),
                ...
                ifelse(space=="Y" & comp=="comp2", 
                      paste("Y\nComponent 2 = ",
                            round(result.spls$explained_variance$Y[2]*100,
                                  digits=1),
                            "%", sep=""),
                # ifelse(space=="Y" & comp=="comp3", 
                #       paste("Y\nComponent 3 = ",
                #             round(result.spls$explained_variance$Y[3]*100,
                #                   digits=1),
                #             "%", sep=""),
                      NA))))) %>% 
  filter(value != 0) %>% 
