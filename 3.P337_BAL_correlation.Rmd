...
enrich.fxn(gene.list = gene.list, ID.type="SYMBOL",
           category = "H",
           genome = "org.Hs.eg.db", 
           basename = "sPLS",
...
```{r echo=FALSE, message=FALSE}
read_csv("results/enrichment/enrich_sPLS_H.csv") %>% 
  bind_rows(read_csv("results/enrichment/enrich_sPLS_C2_CP.csv")) %>% 
  bind_rows(read_csv("results/enrichment/enrich_sPLS_C5_GO.BP.csv")) %>% 
  bind_rows(read_csv("results/enrichment/enrich_sPLS_C7.csv")) %>% 
  group_by(category) %>% 
  slice_min(n=10, order_by = p.adjust) %>% 
  dplyr::select(category, Description, size.overlap.term,
                size.term, p.adjust) %>% 
